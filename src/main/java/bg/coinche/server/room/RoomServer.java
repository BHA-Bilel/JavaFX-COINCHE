package bg.coinche.server.room;

import bg.coinche.MainApp;
import bg.coinche.server.chat.ChatServer;
import bg.coinche.server.game.GameServer;
import bg.coinche.server.game.COINCHE_GameServer;
...
public class RoomServer {
    ...
    public static LocalRoomInfo getJoinRoomInfo() {
        Client host = getHost();
        return (host != null) ? new LocalRoomInfo(room_port, host.name, clients.size()) : null;
    }
    ...
    public void at_migration_finish(MainApp mainApp, boolean exception) {
        new Thread(() -> {
            System.out.println("[server] waiting for migration...");
            while (!migration_finished) Thread.onSpinWait();
            migration_finished = false;
            System.out.println("[server] migration finished!");
    ...
    public synchronized void closeRoom() {
        ...
        if (chatServer != null) chatServer.closeChat();
        if (gameServer != null) gameServer.closeGame();
    }
    ...
    public boolean equals(Object obj) {
        if (RoomServer.this == obj) return true;
    ...
    protected void request_team_up(RoomMsg msg) {
        ...
        if (team_up_with != null) team_up_with.sendMsg(msg);
    }
    ...
    public void team_up(RoomMsg msg) {
        place_mutex.lock();
        Client c1 = clients.get(msg.from);// client who accepted team up request
        Client c2 = clients.get((int) msg.adt_data[0]);// client who requested team up
        ...
        if (c1_teammate != null) c1_teammate.position = c2_pos;
    ...
    public void denied_team_up(RoomMsg msg) {
        ...
        if (rejected != null) rejected.sendMsg(msg);
    }
    ...
    public void NotifyNextPlayer() {
        ...
        if (curr_client != null) curr_client.sendMsg(game_started);
        else endGame();
    }
